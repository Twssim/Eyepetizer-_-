<template>
  <div id="collect">
    ...
    <div class="current-list">
      <ul>
        <li
          @click="jumpDayreport(k.id)"
          v-for="(k, index) in kaiyanDayhandpick"
          :key="index"
        >
          <van-swipe-cell>
            <img :src="k.url" />
...
export default {
  ...
  data() {
    return {
      ...
      title: false,
      obj: [],
      SearchApi: [], //有搜索api提供的视频数据
      SearchVideoRecord: [], //
      kaiyanDayhandpick: [],
    };
  },
  created() {
    this.getVideoRecordFromDayreport();
    this.getVideoRecord();
    this.getVideoRecordFromSearch();
    if (localStorage.getItem("观看视频的记录")) {
      this.obj = JSON.parse(localStorage.getItem("观看视频的记录"));
    }
  },
  methods: {
    movevideo(id) {
      console.log(id);
      console.log(localStorage.getItem("观看视频的记录"));
      for (let i = 0; i < this.obj.length; i++) {
        if (this.obj[i] == id) {
          this.obj.splice(i, 1);
        }
      }
      localStorage.setItem("观看视频的记录", JSON.stringify(this.obj));
      location.reload();
    },
    back() {
      this.$router.go(-1);
    ...
    getVideoRecord() {
      ...
      if (localStorage.getItem("观看视频的记录")) {
        if (localStorage.getItem("成功？")) {
          this.VideoMessage = JSON.parse(
            localStorage.getItem("观看视频的记录")
          );
    ...
    getVideoRecordFromSearch() {
      if (localStorage.getItem("搜索api数据")) {
        ...
        this.VideoMessage = JSON.parse(localStorage.getItem("观看视频的记录"));
        for (let i = 0; i < this.SearchApi.length - 1; i++) {
          this.axios
            ...
            .then((res) => {
              ...
              for (let i = 0; i < res.data.itemList.length - 1; i++) {
                for (let j = 0; j < this.VideoMessage.length - 1; j++) {
                  if (this.VideoMessage[j] == res.data.itemList[i].data.id) {
      ...
      if (localStorage.getItem("搜索api数据")) {
        ...
        for (let i = 0; i < this.SearchApi.length - 1; i++) {
          ...
          this.axios
            ...
            .then((res) => {
              for (let i = 0; i < res.data.itemList.length - 1; i++) {
                for (let j = 0; j < this.obj.length - 1; j++) {
