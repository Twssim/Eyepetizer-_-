<template>
  <div class="video">
    ...
    <div class="videotype">
      ...
      <p v-show="definition" @click="changeLine_click">{{ current_definition === "标清" ? definition_high : "无" }}</p>
    ...
    <div class="video_style">
      ...
      ><!--视频作者名字 !-->
      <p class="videdeScription">{{ description }}</p>
      <!-- 视频注释!-->
...
export default {
  ...
  created() {
    ...
    if (localStorage.getItem("搜索api数据")) {
      this.searchObj = JSON.parse(localStorage.getItem("搜索api数据"));
    }
    ...
    for (let i = 0; i < this.searchObj.length; i++) {
      if (this.searchObj[i] == this.$route.query.value) {
        this.searchObj.splice(i, 1);
      }
    }
    this.searchObj.push(this.$route.query.value);
    localStorage.setItem("搜索api数据", JSON.stringify(this.searchObj));
    //观看视频记录结束层
            ...
            if (res.data.itemList[i].data.id == this.$route.query.id) {
              ...
              if (length > 0) {
                for (let j = 0; j < length; j++) {
                  console.log(res.data.itemList[i].data.playInfo[j]);
...
.video {
  ...
  position: relative;
  overflow-y: scroll;
  background: url("./images/brife-background.jpg");
}
.videotype p {
  width: 1.2rem;
  height: 0.7rem;
